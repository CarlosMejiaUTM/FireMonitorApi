...
export class NodesService {
  ...
  create(createNodeDto: CreateNodeDto, requestingUser: User) {
    ...
    if (requestingUser.role !== UserRole.ADMIN) {
      throw new ForbiddenException(
        'Solo los administradores pueden crear nuevos nodos.',
      );
    }
  ...
  async findOne(id: string, user: User) {
    ...
    if (user.role !== UserRole.ADMIN && node.userId !== user.id) {
      throw new UnauthorizedException(
        'No tienes permiso para acceder a este nodo.',
      );
    }
  ...
  async update(id: string, updateNodeDto: UpdateNodeDto, user: User) {
    if (user.role !== UserRole.ADMIN)
      throw new ForbiddenException(
        'Solo los administradores pueden modificar nodos.',
      );
  ...
  async remove(id: string, user: User) {
    if (user.role !== UserRole.ADMIN)
      throw new ForbiddenException(
        'Solo los administradores pueden eliminar nodos.',
      );
    await this.findOne(id, user);
  ...
  private calculateNodeStatus(node: any): 'alerta' | 'activo' | 'inactivo' {
    ...
    const { fuegoDetectado, temperatura, humoDetectado, concentracionGas } =
      node.ultimaLectura;
    if (
      fuegoDetectado ||
      humoDetectado ||
      (concentracionGas && concentracionGas > 300) ||
      temperatura > 65
    ) {
      return 'alerta';
    }
