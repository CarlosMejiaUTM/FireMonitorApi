...
export class AuthService {
  ...
  async login(loginDto: LoginDto) {
    ...
    if (user && (await bcrypt.compare(loginDto.contrasena, user.contrasena))) {
      // ✅ CORRECCIÓN: Usamos 'fcmToken' en lugar de 'token'
      if (loginDto.fcmToken) {
        await this.usersRepository.updateToken(user.id, loginDto.fcmToken);
      }
  ...
  async resetPassword(
    ...
    const user = await this.usersRepository.findUserByResetToken(
